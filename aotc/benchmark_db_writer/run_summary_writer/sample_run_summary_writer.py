...
def get_db_client(table: str, dataclass_type: Type, is_test: bool = False) -> bq_writer_utils.create_bq_writer_object:
...
def _validate_id(id_value: str,
  ...
  if not result:
    logger.info(
        "%s: %s is not present in the %s table ", id_field.capitalize(), id_value, table_name
    )
    logger.info(
...
def validate_hardware_id(hardware_id: str, is_test: bool = False) -> bool:
  ...
  return _validate_id(hardware_id,
                      "hardware_info",
                      "hardware_id",
...
def validate_software_id(software_id: str, is_test: bool = False) -> bool:
  ...
  return _validate_id(software_id,
                      "software_info",
                      "software_id",
...
def write_run_manually(
    model_id: str, hardware_id: str, software_id: str,
    number_of_nodes: int, number_of_chips: int, container_image_name: str,
    global_batch_size: int, precision: str, optimizer: str, seq_length: int,
    median_step_time: float, e2e_time: float, number_of_steps: int, mfu: float, tokens_per_second: float,
    run_type: str = "perf_optimization",
    run_release_status: str = "local",
    other_metrics_in_json: str = "", nccl_driver_nickname: str = None,
    env_variables: str = "", framework_config_in_json: str = "",
    xla_flags: str = "", topology: str = "", dataset: str = "",
    num_of_superblock: int = None, update_person_ldap: str = os.getenv("USER"),
    comment: str = "", is_test: bool = False
    ) -> None:
  ...
  if (validate_model_id(model_id, is_test) and
      validate_hardware_id(hardware_id, is_test) and
      validate_software_id(software_id, is_test)):

    summary = workload_benchmark_v2_schema.WorkloadBenchmarkV2Schema(
        run_id=f"run-{uuid.uuid4()}",
        experiment_id=f"experiment-manual-{uuid.uuid4()}",
        model_id=model_id,
