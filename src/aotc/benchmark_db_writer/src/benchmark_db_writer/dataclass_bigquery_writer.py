...
from benchmark_db_writer import bigquery_types
from benchmark_db_writer import dataclass_converter_utils
...
class DataclassBigQueryWriter(Generic[T]):
  ...
  def check_schema(self):
    ...
    for table_column, column_attributes in table_schema.items():
      if (
          table_column not in data_schema
          and column_attributes["mode"]
          != bigquery_types.BigQueryFieldModes.NULLABLE
      ):
  ...
  def transform(self, dataclass: T) -> dict:
    return row_transformer_utils.RowTransformer.dataclass_instance_to_bq_row(
        dataclass
    )
  ...
  def _get_query_for_value(
    ...
    if dataclasses.is_dataclass(value):
      value = row_transformer_utils.RowTransformer.dataclass_instance_to_bq_row(
          value
      )
