...
from benchmark_db_writer.schema.supplemental_metrics import data_loading_metrics_schema
from benchmark_db_writer.supplemental_metrics_writer import data_loading_metrics_writer
import pytest
...
def test_write_data_loading_metrics_success(mock_validate_id):
  ...
  try:
    ...
    pytest.fail(
        f"data_loading_metrics_writer() raised unexpected error: {str(e)}"
    )
...
def test_write_data_loading_metrics_invalid_run_id(mock_validate_id):
  ...
  assert (
      str(err_info.value)
      == "Could not upload data in data_loading_metrics table."
  )
