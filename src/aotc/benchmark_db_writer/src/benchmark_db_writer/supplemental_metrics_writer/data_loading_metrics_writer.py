...
_DATA_LOADING_TOKENS_PER_SEC_P50 = flags.DEFINE_float(
    ...
    None,
    "50 percentile of the egress throughput delivered by the storage system"
    " per step, in tokens per second.",
)
_DATA_LOADING_TOKENS_PER_SEC_P90 = flags.DEFINE_float(
    "data_loading_tokens_per_sec_p90",
    None,
    "90 percentile of the egress throughput delivered by the storage system"
    " per step, in tokens per second.",
)
_DATA_LOADING_TOKENS_PER_SEC_P99 = flags.DEFINE_float(
    "data_loading_tokens_per_sec_p99",
    None,
    "99 percentile of the egress throughput delivered by the storage system"
    " per step, in tokens per second.",
)
_DATA_LOADING_TOKENS_PER_SEC_P100 = flags.DEFINE_float(
    "data_loading_tokens_per_sec_p100",
    None,
    "100 percentile of the egress throughput delivered by the storage system"
    " per step, in tokens per second.",
)
_ACCELERATOR_BLOCKED_TIME = flags.DEFINE_float(
    "accelerator_blocked_time",
    None,
    "The duration an accelerator is unavailable for processing tasks due to "
    "data loading.",
)
_ACCELERATOR_BLOCKED_PERCENT = flags.DEFINE_float(
    "accelerator_blocked_percent",
    None,
    "The percent of time an accelerator is unavailable for processing tasks "
    "due to data loading.",
)
...
def write_data_loading_metrics(
  ...
  if bq_writer_utils.validate_id(
    ...
    data_loading_metrics_data = (
        data_loading_metrics_schema.DataLoadingMetricsInfo(
            run_id=run_id,
    ...
    logging.info(
        "Writing Data %s to %s table.", data_loading_metrics_data, table
    )
