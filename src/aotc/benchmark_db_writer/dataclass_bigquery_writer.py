...
from aotc.benchmark_db_writer import bigquery_types
from aotc.benchmark_db_writer import dataclass_converter_utils
from aotc.benchmark_db_writer import row_transformer_utils
from aotc.benchmark_db_writer import bigquery_types
...
def _query_param(
    name: str, schema_field: dict[str, Any], value: Any
):  # -> bigquery._AbstractQueryParameter:
  if schema_field["field_type"] == "STRUCT":
    ...
    return _recursive_struct_param(
        name, schema=schema_field["fields"], values=value
    )
  else:
    return bigquery.ScalarQueryParameter(
        name, schema_field["field_type"], value
    )
...
class DataclassBigQueryWriter(Generic[T]):
  ...
  def __init__(
    ...
    self.input_data_schema = dataclass_converter_utils.dataclass_to_schema(
        self.dataclass_type
    )
    # Get or create table
    try:
  ...
  def check_schema(self):
    ...
    for table_column, column_attributes in table_schema.items():
      if (table_column not in data_schema and
          column_attributes["mode"] != bigquery_types.BigQueryFieldModes.NULLABLE):
  ...
  def update_schema(self):
    ...
    logger.info(
        "DataclassBigQueryWriter: updating schema to %s", pprint.pformat(self.input_data_schema)
    )
    ...
    try:
      ...
      logger.info(
          "BigQueryResultWriter: waiting for some time for the schema to"
          " propagate"
      )
      time.sleep(60)
  ...
  def transform(self, dataclass: T) -> dict:
    return (
        row_transformer_utils.RowTransformer.dataclass_instance_to_bq_row(
            dataclass
        )
    )
  ...
  def read(self, where: Optional[str] = None) -> tuple[list[T], list[T]]:
    ...
    """
    row_transformer = row_transformer_utils.RowTransformer[T](
        self.dataclass_type
    )
  ...
  def _get_query_for_value(
      self, field_name, value
  ):  # -> Tuple[str, bigquery._AbstractQueryParameter]:
    ...
    return f"{field_name} = @{at_name}", _query_param(
        at_name, field_schema, value
    )
  ...
  def write(self, rows: List[T]):
    ...
    try:
      ...
      insert_errors = self.client.insert_rows(
          table=self.table, rows=serialized_rows
      )
      if insert_errors:
        logger.error(
