...
_BASIC_TYPES_TO_NAME = {
    primitive_type: bq_type for bq_type, primitive_type in bigquery_types.TypeMapping.items()
}
_NoneType = type(None)
...
def _python_type_to_big_query_type(field_type: Any) -> bigquery_types.BigQueryTypes:
  if dataclasses.is_dataclass(field_type):
